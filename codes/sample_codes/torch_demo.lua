...
for n1 = 1,train_label1:size(1) do
	train_label[n1] = train_label1[{{n1},{1}}]
end
for n2 = 1,val_label1:size(1) do
	val_label[n2] = val_label1[{{n2},{1}}]
end
train_label1 = nil
val_label1 = nil
collectgarbage()
...
for n = 1,epochs do	
	...
	val_output = model:forward(val_data:cuda())	
    val_err[{{1},{n}}] = criterion:forward(val_output,val_label:cuda())
end
------------Plot performance------------------------------------------
...
-------------Test model------------------------------------------------
sample_test = test_data[{{5,15},{},{},{}}]:cuda()
